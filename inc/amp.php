 ...
 */
function tti_amp_class( $default, $active, $state ) {
	$output = '';
	if( tti_is_amp() ) {
		$output .= sprintf(
			' [class]="%s"',
 ...
 */
function tti_amp_toggle( $state = '', $disable = array() ) {
	if( ! tti_is_amp() )
		return;
 ...
 */
function tti_amp_nav_dropdown( $theme_location = false, $depth = 0 ) {
	...
	return tti_amp_toggle( $key ) . tti_amp_class( 'submenu-expand', 'expanded', $key );
}
